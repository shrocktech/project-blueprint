...
- Beside it, show a compact lifecycle badge from [phases.md](phases.md), such as `RELEASE: beta`; final `PRODUCTION` has no suffix. Distinguish environment and phase with accessible labels and consistent colors. Keep the phase display synchronized with the recorded status through authorized delivery; expose only display-safe values. A keyboard-accessible details control explains the step and completion criteria; plan/issue links must respect admin access. Neither badge controls deployment or access.
