...
- Done means every issue comment addressed, a verified result comment posted, verified behavior, current docs, successful delivery, and a closed issue with only `DONE`. Follow the final comment/closure checks in [git-workflow.md](git-workflow.md); unread or unresolved input blocks closing.
